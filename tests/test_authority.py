...
def test_canonical_codes_match_manuscript_table_4():
    assert int(AuthorityState.ROLLBACK_ISOLATION) == 0
    assert int(AuthorityState.SHADOW_RESTRICTED) == 1
    assert int(AuthorityState.ASSISTED_DEFENSE) == 2
    assert int(AuthorityState.HUMAN_APPROVED) == 3
    assert int(AuthorityState.BOUNDED_AUTOMATION) == 4
